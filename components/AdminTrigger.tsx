...
const AdminTrigger: React.FC<AdminTriggerProps> = ({ onUnlock }) => {
    const { theme, userPassword } = useAppContext();
    ...
    const handlePasswordSubmit = (e: React.FormEvent) => {
        ...
        if (passwordInput === userPassword) {
            ...
            onUnlock('user');
        } else if (passwordInput === MASTER_PASSWORD) {
            setPasswordError('');
            setPasswordInput('');
            setShowPasswordModal(false);
            onUnlock('developer');
                    ...
                    <div className="bg-white dark:bg-zinc-800 rounded-lg shadow-xl p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handlePasswordSubmit}>
                            ...
                            <div className="flex justify-end space-x-2 mt-4">
                                ...
                                <button type="submit" className="px-4 py-2 rounded-md text-sm font-semibold" style={{backgroundColor: theme.accent, color: theme.background}}>Unlock</button>
                            </div>
                        </form>
                    </div>
                </div>
