...
const ServicesPage: React.FC = () => {
  ...
  const { services, showPhotoGallery, showVideoGallery, showMainGallery, galleryItems, heroConfig } = appData;
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {servicePhotos.map(service => (
                  <div key={`photo-${service.id}`} className="group relative overflow-hidden rounded-lg shadow-lg">
                    <img src={service.photoUrl} alt={service.name} className="w-full h-64 object-cover transform group-hover:scale-110 transition-transform duration-300" loading="lazy" decoding="async" />
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            {mainGalleryPhotos.map(item => (
                               <div key={`gallery-photo-${item.id}`} className="group relative overflow-hidden rounded-lg shadow-lg">
                                 <img src={item.url} alt={item.title} className="w-full h-64 object-cover transform group-hover:scale-110 transition-transform duration-300" loading="lazy" decoding="async" />
                                 <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all duration-300 flex items-end p-4">
    ...
    <div style={{ backgroundColor: theme.background, color: theme.textPrimary }} className="pt-32 animate-page-entry">
      <div className="container mx-auto px-6 py-12 md:py-16">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold" style={{ color: theme.textPrimary }}>Our Services</h1>
